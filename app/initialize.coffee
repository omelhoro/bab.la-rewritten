...
$ ->
  Handlebars.registerPartial("sidebar",
    require("views/apps/templates/base/sidebar"))
  Handlebars.registerPartial("slide-meta",
    require("views/apps/templates/base/slide-meta"))
  # a little function for jQuery to select only one and
  # throw error otherwise; something like assert length==1
  $.one = (sel,con)->
    result = $(sel,con)
    if result.length!=1
  ...
  # utility to group elements in
  # jquery according to function or to data attributes
  ...
  $.getLocale = (lang,fallback) ->
    ...
    new Promise (res,rej) ->
      ...
      newLang.then (data) ->
        ...
        #TODO: its a simple parser,
        #too simple( wont work when commas are in a field
